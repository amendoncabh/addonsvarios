odoo.define('pos_loyalty.pos_loyalty', function (require) {
    ...
    models.load_fields('res.partner', ['loyalty_points', 'is_member']);
    ...
    models.load_models([
        {
            ...
            fields: ['name', 'type', 'minimum_points', 'gift_product_id', 'discount_reward_id', 'point_cost', 'discount_product_id', 'discount', 'point_value', 'point_product_id'],
            domain: function (self) {
                return [['loyalty_program_id', '=', self.loyalty.id]];
            },
            loaded: function (self, rewards) {
                self.loyalty_discount_products = [];
                $.each(rewards, function (i, e) {
                    if (e.discount_product_id) {
                        self.loyalty_discount_products.push(e.discount_product_id[0]);
                    }
                });
        ...
        get_won_points: function () {
            ...
            for (var i = 0; i < orderLines.length; i++) {
                ...
                for (var j = 0; j < rules.length; j++) {
                    var rule = rules[j];
                    total_points += Math.floor(line.get_quantity() * rule.pp_product);
                    total_points += Math.floor(line.get_price_with_tax() * rule.pp_currency);
...
//            total_points += Math.floor(total_sold * this.pos.loyalty.pp_currency);
            total_points += Math.floor(total_sold * this.pos.loyalty.pp_currency);
            total_points += Math.floor(product_sold * this.pos.loyalty.pp_product);
            total_points += Math.floor(this.pos.loyalty.pp_order);
            ...
            if (!this.pos.loyalty || !this.get_client()) {
                ...
                for (var i = 0; i < lines.length; i++) {
                    ...
                    if (reward) {
                        ...
                        } else if (reward.type === 'discount_reward') {
                            points += round_pr(line.get_quantity() * reward.point_cost, rounding);
        ...
        get_available_rewards: function () {
            ...
            for (var i = 0; i < this.pos.loyalty.rewards.length; i++) {
                ...
                if (reward.minimum_points > this.get_spendable_points()) {
                    ...
                    continue;
                } else if (reward.type === 'discount_reward' && reward.point_cost > this.get_spendable_points()) {
                    continue;
        ...
        apply_reward: function (reward) {
            ...
            if (!client) {
                ...
                product = this.pos.db.get_product_by_id(reward.discount_reward_id[0]);
            ...
            if (this.pos.loyalty && this.get_client()) {
                ...
                if (client.is_member) {
                    json.loyalty = {
                        ...
                        is_member: this.get_client().is_member,
    ...
    screens.OrderWidget.include({
        update_summary: function () {
            ...
            var client = order.get_client();
            var is_member = (client) ? client.is_member : false;
            if (this.pos.loyalty && order.get_client() && order.get_client().is_member) {
            ...
            setTimeout(function () {
                if (self.pos.loyalty &&
                        self.getParent().action_buttons &&
                        self.getParent().action_buttons.loyalty) {

                    var rewards = order.get_available_rewards();
                    self.getParent().action_buttons.loyalty.highlight((!!rewards.length && is_member));
                }
            });
